...
function OdionFundingsPiChart({data, total}) {
   ...
   function drawChart() {
     const heads = ['Saving', 'SBI Max Gain'];
     ...
     // Remove the old svg
     d3.select('#odion-fundings-pie-container')
       .select('svg')
       .remove();

     // Create new svg
     const svg = d3
       .select('#odion-fundings-pie-container')
       .append('svg')
       .attr('width', width)
       .attr('height', height)
   ...
   return <div id="odion-fundings-pie-container" />;
 }

export default OdionFundingsPiChart;