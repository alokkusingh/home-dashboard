...
import {getCurrentYear, getPreviousMonthDisplay, getPreviousMonthYearMonth} from "./utils/dateUtils";
...
import {
    ...
    fetchExpenseByCategoryYearJson,
    fetchExpenseHeadsJson,
    fetchExpenseMonthsJson,
    fetchExpensesForYearAndCategoryJson,
    fetchExpensesForYearMonthAndCategoryJson,
    fetchExpensesForYearMonthJson,
    fetchMonthlyExpensesForCategoryJson
...
class ExpenseList extends Component {
                    ...
                    const expenseCategoryMonthRows = expensesJson.expenses.map((expense, index) => {
                        return <tr key={expense.id || (yearMonth + '-' + category + '-' + index)}>
                            <td style={{
                                ...
                                textAlign: "Left",
                                fontSize: '.8rem'
                            }}>{format(parseISO(expense.date), 'dd MMM yyyy')}</td>
    ...
    render() {
        ...
        const expenseForCategoriesRows = expensesForCategory.map(record => {
            return <tr key={record.year + '-' + record.month} tranId={record.year + '-' + record.month}
                       category={categoryDropDownValue} onClick={this.showExpenseCategoryModal}>
                <td style={{
                    whiteSpace: 'nowrap',
                    textAlign: "center",
                    fontSize: '.9rem'
        ...
        const expenseByCategoryListRows = filteredExpensesByCategory.map(expense => {
            return <tr key={expense.id} tranId={expense.year + '-' + expense.month} category={expense.category}
                       onClick={this.showExpenseCategoryModal}>
                <td style={{
                    whiteSpace: 'nowrap',
                    textAlign: "center",
                    fontSize: '.9rem'
                }}>{formatYearMonth(expense.year, expense.month)}</td>
        ...
        const yearlyExpenseByCategoryListRows = expensesByYearCategory.map(expense => {
            return <tr key={expense.id || (expense.year + '-' + expense.category)} year={expense.year}
                       category={expense.category} onClick={this.showYearlyExpenseCategoryModal}>
        ...
        return (
            <div id="cards" align="center">
                ...
                <Row>
                    <Col m={6} s={6} l={6}>
                        <div align="left">
                            <ButtonDropdown direction="right" isOpen={categoryDropdownOpenForBar}
                                            toggle={this.toggleCategoryForBar}>
                                ...
                                <DropdownMenu>
                                    {categories.map(e => {
                                        return <DropdownItem id={e} key={e}
                                                             onClick={this.changeCategoryValueForBar}>{e}</DropdownItem>
                                    })}
                                </DropdownMenu>
                            </ButtonDropdown>
                        </div>
                    ...
                    <Col m={2} s={2} l={2}>
                        <div align="left">
                            <ButtonDropdown direction="right" isOpen={monthExpByCatDropdownOpen}
                                            toggle={this.toggleExpMonthByCategory}>
                                <DropdownToggle caret size="sm">
                                    {monthExpByCatDropDownValue}
                                </DropdownToggle>
                                <DropdownMenu>
                                    <DropdownItem id="" key="all-months" onClick={this.changeExpMonthByCategoryValue}>All
                                        Months</DropdownItem>
                                    {months.map(e => {
                                        return <DropdownItem id={e.year + '-' + e.month} key={e.monthStr}
                                                             onClick={this.changeExpMonthByCategoryValue}>{e.monthStr}</DropdownItem>
                                    })}
                                </DropdownMenu>
                            </ButtonDropdown>
                        </div>
                ...
                <Row>
                    <Col m={3} s={3} l={6}>
                        <div align="left">
                            <ButtonDropdown direction="right" isOpen={yearlyDropdownOpenForBar}
                                            toggle={this.toggleYearlyForBar}>
                                <DropdownToggle caret size="sm">
                                    {yearlyDropDownValueForBar}
                                </DropdownToggle>
                                <DropdownMenu>
                                    {categories.map(e => {
                                        return <DropdownItem id={e} key={e}
                                                             onClick={this.changeYearCategoryValueForBar}>{e}</DropdownItem>
                                    })}
                                </DropdownMenu>
                            </ButtonDropdown>
                        </div>
                    ...
                    <Col m={2} s={2} l={2}>
                        <div align="left">
                            <ButtonDropdown direction="right" isOpen={yearExpByCatDropdownOpen}
                                            toggle={this.toggleYearlyExpByCategory}>
                                <DropdownToggle caret size="sm">
                                    {yearExpByCatDropDownValue}
                                </DropdownToggle>
                                <DropdownMenu>
                                    <DropdownItem id="" key="all-years" onClick={this.changeYearlyExpByCategoryValue}>All
                                        Years</DropdownItem>
                                    {years.map(year => {
                                        return <DropdownItem id={year} key={year}
                                                             onClick={this.changeYearlyExpByCategoryValue}>{year}</DropdownItem>
                                    })}
                                </DropdownMenu>
                            </ButtonDropdown>
                        </div>
                ...
                <Row>
                    <Col m={2} s={2} l={2}>
                        <div align="left">
                            <ButtonDropdown direction="right" isOpen={categoryDropdownOpen}
                                ...
                                <DropdownMenu>
                                    {categories.map(e => {
                                        return <DropdownItem id={e} key={e}
                                                             onClick={this.changeCategoryValue}>{e}</DropdownItem>
                                    })}
                                </DropdownMenu>
                            </ButtonDropdown>
                        </div>
                        <Card className="teal lighten-4" textClassName="black-text"
                              title="Monthly Expense for Category">
                            <Table striped bordered hover size="sm">
                                <thead>
                                <tr>
                                    <th width="10%" style={{textAlign: "center"}}>Month</th>
                            ...
                            <Modal isOpen={expenseCategoryModalShow} onClose={this.closeExpenseCategoryModal}
                    ...
                    <Col m={2} s={2} l={2}>
                        <div align="left">
                            <ButtonDropdown direction="right" isOpen={monthExpDropdownOpen}
                                            toggle={this.toggleExpMonth}>
                                <DropdownToggle caret size="sm">
                                    {monthExpDropDownValue}
                                </DropdownToggle>
                                <DropdownMenu>
                                    {months.map(e => {
                                        return <DropdownItem id={e.year + '-' + e.month} key={e.monthStr}
                                                             onClick={this.changeExpMonthValue}>{e.monthStr}</DropdownItem>
                                    })}
                                </DropdownMenu>
                            </ButtonDropdown>
                        </div>
