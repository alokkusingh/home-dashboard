...
import {Col, Modal, ModalHeader, Row, Table} from 'reactstrap';
import {format, parseISO} from 'date-fns';
import {Card, Icon} from 'react-materialize';
import {NumberFormat} from "./utils/NumberFormat";
import {NumberFormatNoCurrency, NumberFormatNoDecimal} from "./utils/NumberFormatUtil";
...
import {
    fetchCurrentMonthExpenseByDayJson,
    fetchExpenseByCategoryForYearJson,
    fetchExpenseByCategoryMonthJson,
    fetchExpenseHeadsJson
} from './api/ExpensesAPIManager.js'
import {fetchMonthlyIncomeExpenseSummaryJson} from './api/SummaryAPIManager.js'
import {fetchInvestmentReturnsProto, fetchInvestmentsForYearJson} from './api/InvestmentAPIManager.js'
            ...
            investmentResponse.forEach(function (record) {
                    yearSummaryModalRows.push(<tr>
                        ...
                        <td style={{
                            ...
                            textAlign: "right",
                            fontSize: '.8rem'
                        }}>{NumberFormat(record.contribution)}</td>
                    </tr>);
                }
            );
        }
    ...
    render() {
        ...
        const monthExpenseList = monthExpenses.map(expense => {
            return <tr key={expense.id} onClick={this.showModal}>
                <td id={expense.id}
                    className="table-cell-center-sm">{format(parseISO(expense.date), 'dd MMM yyyy')}</td>
            ...
            if (expense.year === currentYear && expense.month === currentMonth)
                return <tr key={expense.category} onClick={this.showCategoryExpenseModal}>
                    <td id={expense.category}
                        className="table-cell-center-sm">{formatYearMonth(expense.year, expense.month)}</td>
        ...
        const monthlySummaryList = monthlySummary.map(record => {
            ...
            return <tr key={'' + record.year + record.month} onClick={this.showModal}>
                <td id={'' + record.year + record.month + 0} width="15%" style={{
                    whiteSpace: 'nowrap',
                    textAlign: "center",
                    fontSize: '.75rem'
                }}>{formatYearMonth(record.year, record.month)}</td>
                <td id={'' + record.year + record.month + 1} width="20%"
                    style={{textAlign: "right", fontSize: '.75rem'}}>{NumberFormatNoCurrency(Math.round(income))}</td>
                <td id={'' + record.year + record.month + 2} width="15%" style={{
                    textAlign: "right",
                    fontSize: '.75rem'
                }}>{NumberFormatNoCurrency(Math.round(record.expenseAmount))}</td>
                <td id={'' + record.year + record.month + 3} width="20%" style={{
                    textAlign: "right",
                    fontSize: '.75rem'
                }}>{NumberFormatNoCurrency(Math.round(record.transferAmount))}</td>
                <td id={'' + record.year + record.month + 4} width="15%" style={{
                    textAlign: "right",
                    fontSize: '.75rem'
                }}>{NumberFormatNoCurrency(Math.round(record.investmentAmount))}</td>
                <td id={'' + record.year + record.month + 5} width="15%"
                    style={{textAlign: "right", fontSize: '.75rem'}}>{NumberFormatNoCurrency(Math.round(savings))}</td>
            </tr>
        });
        ...
        yearlySummary.forEach((record, year) => {
            yearlySummaryList.push(<tr key={year} onClick={this.showYearSummaryModal}>
                <td id={year} width="15%"
                    style={{whiteSpace: 'nowrap', textAlign: "center", fontSize: '.75rem'}}>{year}</td>
                <td id={year} width="20%" style={{
                    textAlign: "right",
                    fontSize: '.75rem'
                }}>{NumberFormatNoCurrency(Math.round(record.income) / 1000)}K
                </td>
                <td id={year} width="15%" style={{
                    textAlign: "right",
                    fontSize: '.75rem'
                }}>{NumberFormatNoCurrency(Math.round(record.expenseAmount) / 1000)}K
                </td>
                <td id={year} width="20%" style={{
                    textAlign: "right",
                    fontSize: '.75rem'
                }}>{NumberFormatNoCurrency(Math.round(record.transferAmount) / 1000)}K
                </td>
                <td id={year} width="15%" style={{
                    textAlign: "right",
                    fontSize: '.75rem'
                }}>{NumberFormatNoCurrency(Math.round(record.investmentAmount) / 1000)}K
                </td>
                <td id={year} width="15%" style={{
                    textAlign: "right",
                    fontSize: '.75rem'
                }}>{NumberFormatNoCurrency(Math.round(record.savings) / 1000)}K
                </td>
            </tr>)
        });
        yearlySummaryList.push(<tr key={"Total"}>
            <td id={'Total' + 0} width="15%"
                style={{whiteSpace: 'nowrap', textAlign: "center", fontSize: '.75rem', fontWeight: 900}}>Total
            </td>
            <td id={'Total' + 1} width="20%" style={{
                textAlign: "right",
                fontSize: '.75rem',
                fontWeight: 900
            }}>{NumberFormatNoCurrency(Math.round(total.income) / 1_00_000) + 'L'}</td>
            <td id={'Total' + 2} width="15%" style={{
                textAlign: "right",
                fontSize: '.75rem',
                fontWeight: 900
            }}>{NumberFormatNoCurrency(Math.round(total.expenseAmount) / 1_00_000) + 'L'}</td>
            <td id={'Total' + 3} width="20%" style={{
                textAlign: "right",
                fontSize: '.75rem',
                fontWeight: 900
            }}>{NumberFormatNoCurrency(Math.round(total.transferAmount) / 1_00_000) + 'L'}</td>
            <td id={'Total' + 4} width="15%" style={{
                textAlign: "right",
                fontSize: '.75rem',
                fontWeight: 900
            }}>{NumberFormatNoCurrency(Math.round(total.investmentAmount) / 1_00_000) + 'L'}</td>
            <td id={'Total' + 5} width="15%" style={{
                textAlign: "right",
                fontSize: '.75rem',
                fontWeight: 900
            }}>{NumberFormatNoCurrency(Math.round(total.savings) / 1_00_000) + 'L'}</td>
        </tr>)
        ...
        return (
            <div>
                <div id="cards" align="center">
                    <Row>
                        <Col l={4} m={4} s={4}>
                            <TileCard title="Expense"
                                      x={{
                                          "value": NumberFormatNoCurrency(totalMonthExpense),
                                          "text": getCurrentMonthFull()
                                      }}
                                      a={{
                                          "value": NumberFormatNoCurrency(totalLastMonthExpense),
                                          "text": getPreviousMonthFull()
                                      }}
                                      b={{"value": NumberFormatNoCurrency(totalYearExpense), "text": getCurrentYear()}}
                                      c={{
                                          "value": NumberFormatNoCurrency(totalLastYearExpense),
                                          "text": getPreviousYear()
                                      }}
                            />
                        </Col>
                        <Col l={4} m={4} s={4}>
                            <TileCard title="Investment"
                                      x={{
                                          "value": NumberFormatNoCurrency(investmentTotalValue),
                                          "text": "Market Value"
                                      }}
                                      a={{
                                          "value": NumberFormatNoCurrency(investmentLastMonthReturn),
                                          "text": getPreviousMonthFull() + ' Return'
                                      }}
                                      b={{
                                          "value": NumberFormatNoCurrency(investmentThisYearReturn),
                                          "text": getCurrentYear() + ' Return'
                                      }}
                                      c={{
                                          "value": NumberFormatNoCurrency(investmentCumulativeReturn),
                                          "text": "Total Return"
                                      }}
                            />
                        </Col>
                        ...
                        <Col l={4} m={4} s={4}>
                            <TileCard title="Loan"
                                      ...
                                      a={{
                                          "value": NumberFormatNoDecimal(loanInterestLastMonth),
                                          "text": getPreviousMonthFull() + ' Interest'
                                      }}
                                      b={{
                                          "value": NumberFormatNoDecimal(loanInterestThisYear),
                                          "text": getCurrentYear() + ' Interest'
                                      }}
                                      c={{
                                          "value": NumberFormatNoDecimal(loanInterestLastYear),
                                          "text": getPreviousYear() + ' Interest'
                                      }}
                            />
                        </Col>
                    </Row>
                    ...
                    <Row>
                        <Col m={4} s={4} l={3}>
                            <Card className="card-panel teal lighten-4" closeIcon={<Icon>close</Icon>}
                                ...
                                <Modal isOpen={expenseCatModalShow} onClose={this.closeCategoryExpenseModal}
                                       contentLabel="Expenses" modalClassName="custom-modal-style">
                                    <ModalHeader toggle={this.closeCategoryExpenseModal}>Category
                        ...
                        <Col m={2} s={2} l={2}>
                            <Card className="card-panel teal lighten-4" textClassName="black-text"
                                ...
                                <Modal isOpen={expenseModalShow} onClose={this.closeExpenseModal}
                                       contentLabel="Expenses" modalClassName="custom-modal-style">
                        ...
                        <Col m={4} s={4} l={3}>
                            <Card className="card-panel teal lighten-4" closeIcon={<Icon>close</Icon>}
                                ...
                                <div align="left" style={{fontSize: '.6rem'}}>
                                    Notes: <br/>
                                    1. Inc = Income, which includes salray credited in bank + investment by company -
                                    tax<br/>
                                    2. Exp = Expense <br/>
                        ...
                        <Col m={4} s={4} l={3}>
                            <Card className="card-panel teal lighten-4" closeIcon={<Icon>close</Icon>}
                                ...
                                <Modal isOpen={isYearSummaryModalOpen} onClose={this.closeYearSummaryModal}
                                       contentLabel="YearSummary" modalClassName="custom-modal-style">
