...
import {Button, ButtonContent, ButtonGroup, Divider, Icon, Label, Segment} from 'semantic-ui-react'
...
class RefreshGoogleSheets extends Component {
    ...
    state = {
        ...
        timelineRefreshInProgress: false
    }
    ...
    refreshTimelineGSheet = async (e) => {
        this.setState({timelineRefreshInProgress: true});
        try {
            await refreshSheet('events/life');
        } catch (err) {
            alert("Timeline Refresh failed, error: " + err);
        }
        this.setState({timelineRefreshInProgress: false});
    }

    render() {
        const {
            expenseRefreshInProgress,
            taxRefreshInProgress,
            investmentRefreshInProgress,
            estateRefreshInProgress,
            timelineRefreshInProgress
        } = this.state
        ...
        return (
            <Segment raised color="grey">
                ...
                <Segment inverted color="grey">
                    <ButtonGroup>
                        <Button animated="vertical" loading={expenseRefreshInProgress}
                                onClick={this.refreshExpenseGSheet} color='blue' size='large'>
                            <ButtonContent visible>Expense</ButtonContent>
                            <ButtonContent hidden>
                                <Icon name='refresh'/>
                            </ButtonContent>
                        </Button>
                        <Button animated="vertical" loading={investmentRefreshInProgress}
                                onClick={this.refreshInvestmentGSheet} color='green' size='small'>
                            <ButtonContent visible>Investment</ButtonContent>
                            <ButtonContent hidden>
                                <Icon name='refresh'/>
                            </ButtonContent>
                        </Button>
                        <Button animated="vertical" loading={estateRefreshInProgress} onClick={this.refreshEstateGSheet}
                        ...
                        <Button animated="vertical" loading={taxRefreshInProgress} onClick={this.refreshTaxGSheet}
                        ...
                        <Button animated="vertical" loading={timelineRefreshInProgress} onClick={this.refreshTimelineGSheet}
                                color='purple' size='small'>
                            <ButtonContent visible>Timeline</ButtonContent>
                            <ButtonContent hidden>
                                <Icon name='refresh'/>
                            </ButtonContent>
                        </Button>
